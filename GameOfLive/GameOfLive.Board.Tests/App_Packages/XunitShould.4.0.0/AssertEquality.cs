...
namespace XunitShould
{
    internal static partial class Should
    {
        ...
        public static void ShouldEqual <T>(this IEnumerable <T> actual,
                                           params T[] expected)
        {
            ShouldEqual(actual,
                        expected,
                                           ...
                                           IComparer <T> comparer)
        {
            ...
            var index = 0;
            int lastBoth = Math.Min(actualList.Count,
                                    expectedList.Count);
            for ( ; index < lastBoth ; index++ )
            {
                T actualItem = actualList [ index ];
                T expectedItem = expectedList [ index ];
                if ( comparer.Compare(actualItem,
                                      expectedItem) != 0 )
                {
                    throw new EnumerableEqualException(expectedItem,
                                                       actualItem,
                                                       index,
                                                       expectedList.Count,
                                                       actualList.Count);
            ...
            {
                throw new EnumerableEqualException(expectedList [ index ],
                                                   null,
                                                   index,
                                                   expectedList.Count,
                                                   actualList.Count);
            ...
            {
                throw new EnumerableEqualException(null,
                                                   actualList [ index ],
                                                   index,
                                                   expectedList.Count,
                                                   actualList.Count);
                                       ...
                                       StringComparison comparisonType = StringComparison.CurrentCulture)
        {
            Assert.Equal(expected,
                         actual,
                         comparisonType.GetComparer());
        }

        public static void ShouldEqualWithinPrecision(this double actual,
                                                      double expected,
                                                      int precision)
        {
            Assert.Equal(expected,
        ...
        public static void ShouldEqualWithinPrecision(this decimal actual,
                                                      decimal expected,
                                                      int precision)
        {
            Assert.Equal(expected,
